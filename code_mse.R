...
calculate_posterior_log <- function(y, X, true_theta, sigma, theta_0, Sigma) {
  likelihood <- dnorm(y, mean = X %*% true_theta, sd = sigma,log= TRUE)
  prior <- dmvnorm(true_theta, mean = theta_0, sigma = Sigma, log = TRUE)
  posterior <- likelihood + prior
  return(posterior)
}
calculate_posterior_log(y,X,true_theta,true_sigma,theta_0,Sigma)

# Function to calculate the inverse likelihood (1 / p(y|theta))
calculate_inverse_likelihood <- function(y, X, true_theta, sigma) {
  inverse_likelihood <- 1 / dnorm(y, mean = X %*% true_theta, sd = sigma,log = TRUE)
...
calculate_combined_posterior_log(y,X,theta_0,true_sigma,theta_0,Sigma)
...
theta_samples_from_combined_posterior <- function(y, X, num_samples, theta_0, Sigma, sigma, s.cand) {
  ...
  for (s in 1:num_samples) {
    ...
    combined_posterior_current_log <- calculate_combined_posterior_log(y, X, current_theta, true_sigma, theta_0, Sigma)
    combined_posterior_proposal_log <- calculate_combined_posterior_log(y, X, proposal_theta, true_sigma, theta_0, Sigma)
...
calculate_mixture_estimator <- function(y, X, theta_samples, true_sigma) {
  ...
  for (s in 1:num_samples) {
    ...
    weights <- numeric(num_samples)
      ...
      weighted_likelihoods[s] <- dnorm(y, mean = X %*% t(theta_samples[s, ]), sd = true_sigma) * weights[s]
